...
class GameBoard:
    ...
    # <--------------------------------------- Should I update all character positions? or is it unnecessary?
    # _raccoon:
    #   spawned raccoon in the game
                    # _smart_raccoon:                     <--- Delete since rules apply the same to smart raccoon and raccoon
                    #   spawned smart raccoon in the game
    # _garbage_can:
    #   spawned garbage can in the game
    # _recyclin_bin:
    #   spawned recyclin bin in the game
    # _map:
    #   the entire map of the game
    ...
    # _smart_raccoon: List[SmartRaccoon]
    _garbage_can: List[GarbageCan]
    _recyclin_bin: List[RecyclingBin]
    ...
    def __init__(self, w: int, h: int) -> None:
        ...
        >>> b._raccoon is None
        True
        >>> b._smart_raccoon is None
        True
        >>> b._garbage_can is None
        True
        >>> b._recyclin_bin is None
        True
        >>> b._map == [['-', '-', '-'], ['-', '-', '-'], ['-', '-', '-']]
        True
        """
    ...
    def place_character(self, c: Character) -> None:
        ...
        """
        # TODO Task #1
        # for test case: verify with GameBoard.at
        # check character type
        # Only Raccoon and GarbageCan can be together

        # check what character is <c>
        # check if any character is already in the spot
        # if present check <c> GarbageCan, and <c> is raccoon
        #   check if GarbageCan is Locked
    ...
    def to_grid(self) -> List[List[chr]]:
        ...
        """
        # TODO
        ### Assume Garbage can cannot be placed after raccoon has been placed first
        map_ = []
        for i in self._map:
            row = []
            for c in i:
    ...
    def adjacent_bin_score(self) -> int:
        ...
        """
        """
        Say we dfs with ((x, y), connected: bool)
        Cases:
        i) (x,y) = 'B' & Coonnected
            count
        ii) (x, y) = 'B' & Not Connected <-- if q pop this, it means it already traversed one clustered
            don't count, strat brand new
        iii) (x,y) != 'B'
            skip
        """
        # TODO Task #5
        # connected been needs to be checked left and right
        # we can use dfs
        grid_ = self.to_grid()
        q = [(0, 0, True)]
        all_connections = []
    ...
    def move_character(self, c: Character, prev_x: int, prev_y: int) -> None:
        ...
        """
        # note that currently this method only works for Player and RecylcliBin
        # for Raccoons it should be little different since it can move away from
        # Garbage Can
        if c.get_char() in ['P', 'B']:
            ...
            if self._map[c.y][c.x] == '-':
                self._map[c.y][c.x] = [c]
                    ...
                    len(self._map[c.y][c.x]) == 1:
                self._map[c.y][c.x].append(c)
        # This only activates when Player or Raccoon unlocks Garbage Can
        # elif c.get_char() == 'C':
        #     if self._map[prev_y][prev_x][0].get_char() == 'C'
        #         self._map[prev_y][prev_x] =
...
class Raccoon(TurnTaker):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        # if Raccoon is already inside the bin. Whether it is an
        # Unlocked Garbage Can or Unlocked Garbage Can
        # I'm assuming Garbage Can will not automatically be locked when
        # Raccoon goes in.
        # Player can also lock an Unlocked Garbage can with Raccoon in it
        if self.inside_can:
            return False
        if self.board.on_board(move_x, move_y):  # coordinate inside the board
            ...
            if not self.board.at(move_x, move_y):
                ...
                # self.board.move_character(locked_garbage,
                #                           locked_garbage.x, locked_garbage.y)
                return True
            elif self.board.at(move_x, move_y)[0].get_char() == \
                    ('R' or '@' or 'P' or 'B'):
                return False
...
class SmartRaccoon(Raccoon):
    ...
    def take_turn(self) -> None:
        ...
        """
        # TODO Task #4
        # stretch search till the end of the board in all 4 directions
        # Check all direction and check if garbage can and if nothing blocks
        # except player
        ...
        while left_possible or up_possible or right_possible or down_possible:
            # check left:
            # print(f'LEFT: {self.board.on_board(self.x - i, self.y)}')
            if self.board.on_board(self.x - i, self.y) and left_possible:
                if grid_[self.y][self.x - i] in go_towards:
                    potential_d = LEFT
                    break
                elif grid_[self.y][self.x - i] in blockers:
                    left_possible = False
            else:
                left_possible = False
            # check up:
            # print(f'UP: {self.board.on_board(self.x, self.y - i)}')
            if self.board.on_board(self.x, self.y - i) and up_possible:
                if grid_[self.y - i][self.x] in go_towards:
                    potential_d = UP
                    break
                elif grid_[self.y - i][self.x] in blockers:
                    up_possible = False
            else:
                up_possible = False
            # check right:
            # print(f'RIGHT: {self.board.on_board(self.x + i, self.y)}')
            if self.board.on_board(self.x + i, self.y) and right_possible:
                if grid_[self.y][self.x + i] in go_towards:
                    potential_d = RIGHT
                    break
                elif grid_[self.y][self.x + i] in blockers:
                ...
                right_possible = False
            # check down:
            # print(f'DOWN: {self.board.on_board(self.x, self.y + i)}')
            if self.board.on_board(self.x, self.y + i) and down_possible:
                if grid_[self.y + i][self.x] in go_towards:
                    potential_d = DOWN
                    break
                elif grid_[self.y + i][self.x] in blockers:
        ...
        # using grid_[curr_y][curr_x], check 4 directions
        # check right
        potential_d = []
        i = 1
        # for c in ['R', 'S', 'B', 'O', 'C']:
        #     # check left:
        #     if self.board.on_board(curr_x - i, curr_y):
        #         if c == grid_[curr_y][curr_x - i] and \
        #                 c in ['O', 'C']:
        #             potential_d = LEFT
        #             break
        #     # check up:
        #     if self.board.on_board(curr_x, curr_y - i * self.board.width):
        #         if c == grid_[curr_y - i * self.board.width][curr_x] and \
        #                 c in ['O', 'C']:
        #             potential_d = UP
        #             break
        #     # check right:
